...
const educations: EducationItem[] = [
  {
    ...
    schoolLink: '#',
    period: 'Aug 2013 - June 2018',
    description: 'I graduated with a bachelor\'s degree in Information Technology Engineering, and I decided to work as a Web Developer.',
    highlights: [
      ...
      'My graduation thesis received a 9.7/10, making it the third highest mark in the university. That particular moment made me really happy.\nYou may view that moment of mine here: https://github.com/DoAnTotNghiepTeam'
    ]
  }
];
            ...
            <div className='flex justify-between items-start mb-2'>
              <div>
                <h3 className='text-2xl font-bold text-teal-600 flex items-center gap-2'>
                  {edu.school}
                  {edu.schoolLink && <LinkOutlined className='text-lg' />}
                </h3>
              </div>
              <p className='text-right text-lg font-semibold text-teal-600'>{edu.period}</p>
            </div>

            {/* Description */}
            <p className='text-gray-700 mb-4'>
              <span className='font-semibold text-black'>• {edu.description}</span>
            </p>
                  ...
                  <div>
                    {highlight.split('\n').map((line, i) => (
                      <div key={i}>
                        {line.includes('https://') ? (
                          <>
                            You may view that moment of mine here:{' '}
                            <a 
                              href={line.split(': ')[1]} 
                              target='_blank' 
